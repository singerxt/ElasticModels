 ...
 * @
 */
const find = ({ elasticSearchClient, index, type, schema }) => console.log('siemano') || ({
  find: (query = {}, options = {}) => elasticSearchClient.search({
    ...
    from: options.from || 0,
    body: query
  }).then(results => documentArray(results, schema, options))
})
...
const findById = ({ elasticSearchClient, index, type, schema, options }) => ({
  findById: id => elasticSearchClient.get({
    ...
    type,
    id
  }).then(results => document(results, schema, options))
})
